...
const getLiveMatches = async (_req, res) => {
    try {
        ...
        res.json((0, toSnake_1.default)(enrichedMatches));
    }
    catch (error) {
        const err = error;
...
const getMatchById = async (req, res) => {
    try {
        ...
        const team1 = await database_1.default.team.findFirst({
            where: { teamName: match.team1Name },
            select: { logoUrl: true, _count: { select: { teamPlayers: true } } },
        });
        const team2 = await database_1.default.team.findFirst({
...
const createMatch = async (req, res) => {
    ...
    try {
        ...
        res.status(201).json((0, toSnake_1.default)(match));
    }
    catch (error) {
        const err = error;
        res.status(500).json({ error: err.message });
...
const updateMatch = async (req, res) => {
    ...
    try {
        ...
        if (status)
            data.status = status;
        if (toss_winner)
            data.tossWinner = toss_winner;
        if (toss_decision)
            data.tossDecision = toss_decision;
        if (winner)
            data.winner = winner;
        if (current_innings !== undefined)
            data.currentInnings = current_innings;
        if (umpire !== undefined)
            data.umpire = umpire || null;
        if (player_of_match !== undefined)
            data.playerOfMatch = player_of_match || null;
        ...
        res.json((0, toSnake_1.default)(match));
    }
    catch (error) {
        const err = error;
        res.status(500).json({ error: err.message });
...
async function updateTournamentStandings(tournamentId) {
    ...
    const tournamentTeams = await database_1.default.tournamentTeam.findMany({
        where: { tournamentId },
        include: { team: true },
    });
    ...
    await Promise.all(tournamentTeams.map((tt) => {
        ...
        return database_1.default.tournamentTeam.update({
            where: { id: tt.id },
            data: {
                played: s.played,
                won: s.won,
...
const getScorecard = async (req, res) => {
    try {
        ...
        const batting = await database_1.default.playerScore.findMany({
            where: {
        ...
        const bowling = await database_1.default.playerScore.findMany({
            where: {
                matchId: parseInt(req.params.id),
                oversBowled: { gt: 0 },
            },
        ...
        res.json((0, toSnake_1.default)({ match, batting: batting.map(flattenScore), bowling: bowling.map(flattenScore) }));
    }
    catch (error) {
        const err = error;
        res.status(500).json({ error: err.message });
